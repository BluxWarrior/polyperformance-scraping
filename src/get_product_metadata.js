...
async function get_product_metadata(numberofprocess = 4) {
  ...
  for (let i = 0; i < numberofprocess; i++) products["data"].push([]);
  // check if the mata file exists
  if (fs.existsSync("./assets/metadata.json"))
    products = JSON.parse(fs.readFileSync("./assets/metadata.json", "utf8"));
    ...
    while (!finished) {
      ...
      try {
        for (const bd of brands.slice(
          chunksize * processnumber + products["data"][processnumber].length,
  ...
  await Promise.all(processes).then(() => {
    ...
    let data = [];
    for (const subcat of products["data"]) data = data.concat(subcat);
    products["data"] = data;
