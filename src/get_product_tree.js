...
async function get_product_tree() {
  ...
  while (!finished) {
    try {
      ...
      if (fs.existsSync("./assets/tree.json"))
        data = JSON.parse(fs.readFileSync("./assets/tree.json", "utf8"));
      ...
      for (const ct of categories.slice(data.length)) {
        ...
        fs.writeFileSync("./assets/tree.json", jsonContent, "utf8");
      }
      ...
      fs.writeFileSync("./assets/tree_table.json", jsonContent, "utf8");
