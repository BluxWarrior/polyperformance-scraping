...
async function get_progress() {
  ...
  if (fs.existsSync(`./assets/brands.json`)) {
    ...
    numberofbrands = JSON.parse(fs.readFileSync("./assets/brands.json", "utf8")).length;
  } else {
    return progress
  }
  ...
  if (fs.existsSync(`./assets/metadata.json`)) {
    progress["metadata"] = JSON.parse(fs.readFileSync("./assets/metadata.json", "utf8")).length / numberofbrands * 100;
  } else {
    return progress
  }

  // get details process
  if (fs.existsSync(`./assets/data`) && progress["metadata"] == 100.0) progress["details"] = fs.readdirSync("./assets/data").length / numberofbrands * 100;
